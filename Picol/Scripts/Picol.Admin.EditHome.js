$(window).load(function () {
    ...
    $("#HomeHeader").click(function () {
        $("input[id='HomeHeaderFile']").click();
    });

    $("#HomeHeaderFile").change(function () {
        var formData = new FormData();
        formData.append('file', $("input[id='HomeHeaderFile']")[0].files[0]);

        $.ajax({
            type: 'POST',
            url: '/Admin/UploadHomeHeader',
            data: formData,
            ...
            success: function (response) {
                if (response.Error) {
                    ...
                    $(".HomeHeaderPreview").attr('src', url);
                }
            },
