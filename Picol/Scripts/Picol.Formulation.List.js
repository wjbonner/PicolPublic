$.FormulationHelper = {
    QueueAndExecuteCommand: function (item, column, editCommand) {
        if (column['id'] == 'Name') {
            $.Formulation.Update(item.id, editCommand.serializedValue, item.Code, item.Notes, editCommand);
        } else if (column['id'] == 'Code') {
            $.Formulation.Update(item.id, item.Name, editCommand.serializedValue, item.Notes, editCommand);
        } else if (column['id'] == 'Notes') {
            $.Formulation.Update(item.id, item.Name, item.Code, editCommand.serializedValue, editCommand);
        } else {
            editCommand.execute();
            $('#OverlayDisplay').hide();
        }
    ...
    ActionsFormatter: function (row, cell, value, columnDef, dataContext) {
        var details = "<a href='/Formulation/Details/" + dataContext["id"] + "'>Details</a>";
        var del = "<a href='javascript:void(0)' onclick='$.Formulation.Delete(" + dataContext["id"] + ")'>Delete</a>";
        return details + " | " + del;
    }
}

$.Formulation = {
    CookieName: "Picol.Formulation.List.Grid",
    ...
    Columns: [
        ...
        { id: "Actions", name: "Actions", field: "Actions", sortable: true, formatter: $.FormulationHelper.ActionsFormatter }
    ],
    ...
    ResizeGridContainer: function () {
        $('#' + $.Formulation.GridContainer).height($(window).height() - 35);
    },

    Create: function () {
        $.ajax({
            type: 'POST',
            url: '/Formulation/Create',
            ...
            success: function (response) {
                if (response.Error) {
                    ...
                    $.Formulation.DataView.addItem({
                        id: response.Formulation.Id,
                        Name: response.Formulation.Name,
                        Code: response.Formulation.Code,
                        Notes: response.Formulation.Notes,
                    });
    ...
    Delete: function (id) {
        if (!confirm('Are you sure you want to permenantly and irrevocably delete this formulation and all associated records?')) {
            return;
        }

        $.ajax({
            type: 'POST',
            url: '/Formulation/Delete',
            data: { id: id },
            dataType: "json",
            success: function (response) {
                if (response.Error) {
                    ...
                    $.Formulation.DataView.deleteItem(id);
                }
            },
            error: function (response) {
                alert('An error has occurred communicating with the server!');
