$.SignalWordHelper = {
    QueueAndExecuteCommand: function (item, column, editCommand) {
        if (column['id'] == 'Name') {
            $.SignalWord.Update(item.id, editCommand.serializedValue, item.Code, editCommand);
        } else if (column['id'] == 'Code') {
            $.SignalWord.Update(item.id, item.Name, editCommand.serializedValue, editCommand);
        } else {
            editCommand.execute();
            $('#OverlayDisplay').hide();
        }
    ...
    ActionsFormatter: function (row, cell, value, columnDef, dataContext) {
        var details = "<a href='/SignalWord/Details/" + dataContext["id"] + "'>Details</a>";
        var del = "<a href='javascript:void(0)' onclick='$.SignalWord.Delete(" + dataContext["id"] + ")'>Delete</a>";
        return details + " | " + del;
    }
}

$.SignalWord = {
    CookieName: "Picol.SignalWord.List.Grid",
    ...
    Columns: [
        ...
        { id: "Actions", name: "Actions", field: "Actions", sortable: true, formatter: $.SignalWordHelper.ActionsFormatter }
    ],
    ...
    ResizeGridContainer: function () {
        $('#' + $.SignalWord.GridContainer).height($(window).height() - 35);
    },

    Create: function () {
        $.ajax({
            type: 'POST',
            url: '/SignalWord/Create',
            ...
            success: function (response) {
                if (response.Error) {
                    ...
                    $.SignalWord.DataView.addItem({
                        id: response.SignalWord.Id,
                        Name: response.SignalWord.Name,
                        Code: response.SignalWord.Code
                    });
    ...
    Delete: function (id) {
        if (!confirm('Are you sure you want to permenantly and irrevocably delete this signal word and all associated records?')) {
            return;
        }

        $.ajax({
            type: 'POST',
            url: '/SignalWord/Delete',
            data: { id: id },
            dataType: "json",
            success: function (response) {
                if (response.Error) {
                    ...
                    $.SignalWord.DataView.deleteItem(id);
                }
            },
            error: function (response) {
                alert('An error has occurred communicating with the server!');
