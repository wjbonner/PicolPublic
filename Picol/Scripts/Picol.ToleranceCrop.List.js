$.ToleranceCropHelper = {
    QueueAndExecuteCommand: function (item, column, editCommand) {
        if (column['id'] == 'Name') {
            $.ToleranceCrop.Update(item.id, editCommand.serializedValue, item.Code, item.Notes, editCommand);
        } else if (column['id'] == 'Code') {
            $.ToleranceCrop.Update(item.id, item.Name, editCommand.serializedValue, item.Notes, editCommand);
        } else if (column['id'] == 'Notes') {
            $.ToleranceCrop.Update(item.id, item.Name, item.Code, editCommand.serializedValue, editCommand);
        } else {
            editCommand.execute();
            $('#OverlayDisplay').hide();
        }
    ...
    ActionsFormatter: function (row, cell, value, columnDef, dataContext) {
        var details = "<a href='/ToleranceCrop/Details/" + dataContext["id"] + "'>Details</a>";
        var del = "<a href='javascript:void(0)' onclick='$.ToleranceCrop.Delete(" + dataContext["id"] + ")'>Delete</a>";
        return details + " | " + del;
    }
}

$.ToleranceCrop = {
    CookieName: "Picol.ToleranceCrop.List.Grid",
    ...
    Columns: [
        ...
        { id: "Actions", name: "Actions", field: "Actions", sortable: true, formatter: $.ToleranceCropHelper.ActionsFormatter }
    ],
    ...
    ResizeGridContainer: function () {
        $('#' + $.ToleranceCrop.GridContainer).height($(window).height() - 35);
    },

    Create: function () {
        $.ajax({
            type: 'POST',
            url: '/ToleranceCrop/Create',
            ...
            success: function (response) {
                if (response.Error) {
                    ...
                    $.ToleranceCrop.DataView.addItem({
                        id: response.ToleranceCrop.Id,
                        Name: response.ToleranceCrop.Name,
                        Code: response.ToleranceCrop.Code,
                        Notes: response.ToleranceCrop.Notes,
                    });
    ...
    Delete: function (id) {
        if (!confirm('Are you sure you want to permenantly and irrevocably delete this tolerance crop and all associated records?')) {
            return;
        }

        $.ajax({
            type: 'POST',
            url: '/ToleranceCrop/Delete',
            data: { id: id },
            dataType: "json",
            success: function (response) {
                if (response.Error) {
                    ...
                    $.ToleranceCrop.DataView.deleteItem(id);
                }
            },
            error: function (response) {
                alert('An error has occurred communicating with the server!');
